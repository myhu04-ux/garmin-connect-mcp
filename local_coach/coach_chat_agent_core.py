"""Core tool/model router for the local Garmin coach chat.
...
free-form LLM text. Time-sensitive/deep coaching first checks that local Garmin data
are fresh enough for the answer.
"""
...
def ensure_current_data(label: str, max_age_minutes: int = 30) -> str | None:
    stage(label)
    try:
        result = data_freshness.ensure_fresh(max_age_minutes=max_age_minutes)
    except Exception as exc:
        return (
            "Jeg vil ikke lave en personlig vurdering på forældede Garmin-data. "
            f"Den read-only opdatering kunne ikke bekræftes: {exc}"
        )
    if result.get("refreshed"):
        stage("Friske Garmin-data er klar; fortsætter analysen…")
    elif result.get("waited_for_other_process"):
        stage("En anden Garmin-opdatering blev færdig; fortsætter med de friske data…")
    return None


def needs_fresh_direct(message: str) -> bool:
    text = " ".join(message.casefold().strip().split())
    phrases = (
        "restitution", "hrv", "søvn", "soevn", "hvilepuls", "body battery", "stress i dag",
        "næste pas", "naeste pas", "kommende pas", "hvad skal jeg træne", "hvad skal jeg traene",
        "hvad træner jeg", "hvad traener jeg", "træne i dag", "traene i dag",
    )
...
def shadow_week_answer(message: str) -> str:
    freshness_error = ensure_current_data("Henter friske Garmin-data til ugeanalysen…", max_age_minutes=30)
    if freshness_error:
...
def deep_answer(message: str) -> str:
    freshness_error = ensure_current_data("Henter friske Garmin-data til flerugersanalysen…", max_age_minutes=30)
    if freshness_error:
        return freshness_error
    stage(
...
def answer(message: str) -> str:
    ...
    if needs_fresh_direct(message):
        freshness_error = ensure_current_data("Kontrollerer at dagens Garmin-data er friske…", max_age_minutes=30)
        if freshness_error:
            return freshness_error
